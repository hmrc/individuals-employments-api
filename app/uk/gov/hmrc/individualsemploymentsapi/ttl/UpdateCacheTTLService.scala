...
package uk.gov.hmrc.individualsemploymentsapi.ttl
...
import uk.gov.hmrc.individualsemploymentsapi.cache.v2.CacheRepositoryConfiguration
...
class UpdateCacheTTLService @Inject() (
  ...
  private def updateItem(collection: MongoCollection[Document]): Future[Unit] =
    acquireLock().flatMap {
      case true =>
        logger.info(s"Lock acquired. Starting aggregation-based update.")
