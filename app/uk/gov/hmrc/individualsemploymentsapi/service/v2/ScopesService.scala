...
import uk.gov.hmrc.individualsemploymentsapi.config.{ApiConfig, EndpointConfig}
...
class ScopesService @Inject()(configuration: Configuration) {
  ...
  private[service] def getFieldNames(keys: Iterable[String]): Iterable[String] =
    apiConfig.endpoints
      .map(e => e.fields)
  ...
  private[service] def getAccessibleEndpoints(scopes: Iterable[String]): Iterable[String] = {
