...
abstract class CommonController @Inject() (cc: ControllerComponents) extends BackendController(cc) {
  ...
  private[controller] def urlWithInterval[T](url: String, fromDate: LocalDateTime)(implicit request: Request[T]) = {
  ...
  private[controller] def withAudit(correlationId: Option[String], matchId: String, url: String)(implicit
