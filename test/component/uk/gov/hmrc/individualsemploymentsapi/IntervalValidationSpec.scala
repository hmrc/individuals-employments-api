...
class IntervalValidationSpec extends BaseSpec {
  ...
  feature("Date interval query parameter validation") {
    ...
    scenario("missing fromDate parameter") {
      ...
      val response = Http(s"$serviceUrl/sandbox/paye/match?matchId=$matchId&toDate=2017-03-01")
    ...
    scenario("invalid format for fromDate parameter submitted") {
      ...
      val response = Http(s"$serviceUrl/sandbox/paye/match?matchId=$matchId&fromDate=20160101&toDate=2017-03-01")
        .headers(requestHeaders(acceptHeaderVP1)).asString
    ...
    scenario("invalid format for toDate parameter submitted") {
      ...
      val response = Http(s"$serviceUrl/sandbox/paye/match?matchId=$matchId&fromDate=2016-01-01&toDate=20170301")
        .headers(requestHeaders(acceptHeaderVP1)).asString
    ...
    scenario("invalid interval submitted. ToDate value before fromDate") {
      ...
      val response = Http(s"$serviceUrl/sandbox/paye/match?matchId=$matchId&fromDate=2017-01-01&toDate=2016-03-01")
        .headers(requestHeaders(acceptHeaderVP1)).asString
