...
class SandboxEmploymentsControllerSpec extends BaseSpec {
  ...
  feature("View individuals root (hateoas) entry point is accessible") {
    ...
    scenario("valid request to the sandbox implementation") {
      ...
      val response = Http(s"$serviceUrl/sandbox?matchId=57072660-1df9-4aeb-b4ea-cd2d7f96e430").headers(requestHeaders()).asString

      Then("The response status should be 200 (OK)")
      response.code shouldBe OK
      response.body shouldBe
        """{"_links":{"paye":{"href":"/individuals/employments/paye?matchId=57072660-1df9-4aeb-b4ea-cd2d7f96e430{&fromDate,toDate}","title":"View individual's employments"},"self":{"href":"/individuals/employments/?matchId=57072660-1df9-4aeb-b4ea-cd2d7f96e430"}}}"""
    }
  }
